...
async function init() {
  ...
  try {
    ...
    await primeStartupSourcePreview(manifest);
...
async function primeStartupSourcePreview(manifest) {
  const manifestTargetFile = manifest && manifest.target && manifest.target.file;
  if (!manifestTargetFile) {
    revealStartupSourcePreviewFallback();
    return;
  }

  const targetUrl = new URL(manifestTargetFile, state.manifestUrl);
  try {
    await loadImageElement(targetUrl.href);
    setStartupSourcePreview(targetUrl);
    return;
  } catch {
    revealStartupSourcePreviewFallback();
  }
...
function setStartupSourcePreview(url) {
  dom.sourcePreview.classList.remove('fading-out');
  dom.sourcePreview.src = url.href;
  state.currentPreviewPath = url.href;
  dom.sourcePreview.style.visibility = '';
}

function revealStartupSourcePreviewFallback() {
  dom.sourcePreview.style.visibility = '';
}
